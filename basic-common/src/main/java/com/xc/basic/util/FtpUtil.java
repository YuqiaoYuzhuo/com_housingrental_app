...
import com.xc.basic.enums.FileEnum;
import com.xc.basic.exception.BusinessException;
import com.xc.basic.model.FtpModel;
...
//    }
    ...
    public void connectServer(FtpModel ftm)
    {
        ...
        {	
            ...
            if(islogin){
                 ...
                 if (ftm.getRemotePath() != null && ftm.getRemotePath().length() > 0 && !"/".equals(ftm.getRemotePath()))
                 {
            	  ...
            	  logger.error(":登录失败!");
                  throw new BusinessException("登录服务器失败,请检测服务器配置,若有疑问请联系管理员!");
            }
        ...
        catch (SocketException e)
        {
            ...
            throw new BusinessException("连接服务器失败,请检测服务器配置,若有疑问请联系管理员!");
    ...
    public boolean CreateDirecroty(String remote,FTPClient ftpClient) throws IOException{   
        String directory = CommonFileUtils.getFileDirectroyPath(remote);   
         if(!directory.equalsIgnoreCase("/")&&!ftpClient.changeWorkingDirectory(CommonFileUtils.encodedFileName(directory))){   
            ...
            while(true){   
                String subDirectory = new String(CommonFileUtils.encodedFileName(remote.substring(start,end)));   
    ...
    public String uploadFile(UploadFileModel ufm) throws IOException
    {
        if (ufm != null && null!=ufm.getFile())
        {
        	...
        	if(status){
            	...
            	logger.debug("文件:["
        		+CommonFileUtils.getFileNameByPath(ufm.getFileSystemName())+"]上传成功！总耗时："
        		+(endTime-beginTime)+"ms;"+"文件大小：["+CommonFileUtils.GetFileSize(ufm.getFile())+"]"
        		);
            	  //清空缓存目录
    ...
    private boolean localUpload(UploadFileModel ufm,String rootPath) {
    	...
    	String path= CommonFileUtils.getFileDirectroyPath(ufm.getFile_upload_real_path());
    ...
    private boolean remoteUpload(UploadFileModel ufm){  
    	...
    	Integer result = null;
    	//本地临时路径
    	String local = ftm.getUploadTempDir()+CommonFileUtils.getFileNameByPath(ufm.file_upload_real_path);
    	//远程路径
    	...
    	try {
			...
			if(remote.contains("/")){   
			    remoteFileName = CommonFileUtils.getFileNameByPath(remote);   
			...
			//检查远程是否存在文件   
			FTPFile[] files = ftpClient.listFiles(CommonFileUtils.encodedFileName(remote));   
			if(files.length == 1){   
	...
	try {
		if("true".equals(ftm.getIsRemote())){
	        ...
	        //设置以二进制方式传输   
				ftpClient.setFileType(FTP.BINARY_FILE_TYPE);
				  //检查远程文件是否存在   
		        FTPFile[] files = ftpClient.listFiles(CommonFileUtils.encodedFileName(fileName));  
		        if(files.length != 1){   
		        ...
		        //创建临时文件
				 String name= CommonFileUtils.getFileNameByPath(fileName);
		        ...
		        if(resFile.exists()){   
		            ...
		            InputStream in = ftpClient.retrieveFileStream(CommonFileUtils.encodedFileName(fileName));  
		            //传输进度监控
		...
		} finally
        {
           ...
           logger.debug("---下载文件大小：["+CommonFileUtils.GetFileSize(resFile)+"]");
        }
		return resFile;
	}
	/**
	...
	public void deleteFile(String fileName)
    {
        ...
        try
        {
        	long beginTime = System.currentTimeMillis();
        	//判断是否是远程上传
        	 String path= CommonFileUtils.getFileDirectroyPath(fileName);
             String name =  CommonFileUtils.getFileNameByPath(fileName);
        	if("true".equals(ftm.getIsRemote())){
        		//连接服务器
                this.connectServer(ftm);
                ftpClient.changeWorkingDirectory(CommonFileUtils.encodedFileName(path));
                if(!ftpClient.deleteFile(CommonFileUtils.encodedFileName(name))) throw new BusinessException("删除文件失败！");
            }else{
            	//本地删除
	...
	private Integer uploadFile(String remoteFile,File localFile,FTPClient ftpClient,long remoteSize) throws IOException{   
        ...
        //显示进度的上传   
        long step =  CommonFileUtils.getFileSize(localFile)/ 100;   
        long process = 0;   
        long localreadbytes = 0L;   
        RandomAccessFile raf = new RandomAccessFile(localFile,"r");   
        OutputStream out = ftpClient.appendFileStream(CommonFileUtils.encodedFileName(remoteFile));   
        //断点续传   
        if(remoteSize>0){   
	...
	private Map<String,Object>  getFileListRemote(String path,FtpModel ftm){
		  List<UploadFileModel> fileLists = new ArrayList<UploadFileModel>();
	        ...
	        for (int i = 0; ftpFiles != null && i < ftpFiles.length; i++)
	        {
	        	...
	        	String fileRealName=CommonFileUtils.decodedFileName(file.getName());
	        	upm.setFileRealName(fileRealName);
	            if (file.isFile())
	            {  
	                fileLists.add(upm);
